...
function createzLeaf($trimarray,$data,$T_photo,$img,$ordersub_id='',$newPage){
	...
	if(!$newPage){
		logging_run($xy,'准备更新照片页的Trim值--老页更新:',"photobook");
	...
	logging_run($ordersub_id,'订单页id','photobook');
	...
	foreach ($data as $key => $invalue) {
		...
		if($invalue['type']=='img'){
			...
			if(empty($U_photo)){
				logging_run('订单页id：'.$ordersub_id.'  key为：'.$key.' invalue'.json_encode($invalue),'error','photobook');
			}
			$U_size = getimagesize($U_photo);
			...
			if(!$newPage){//如果是新建页，则计算默认值
				logging_run($ordersub_id,'此时为旧页，从数据库取Xleft和XTop值:',"photobook");
				$klt=array(
				'x'=>trim($trimarray[$key]['Xleft'],'px'),
				'y'=>trim($trimarray[$key]['Xtop'],'px'),
				);
			}else{
				logging_run($ordersub_id,'此时为新页，系统按默认值计算位置，id为:',"photobook");
				$klt=array(
				'x'=>-abs($inxy[0]-$w)/2,
				'y'=>-abs($inxy[1]-$h)/2,
				);
	...
	if($newPage){
		$trim=json_encode($trimarray);
		logging_run($xy,'准备更新照片页的Trim值--新页首次更新:',"photobook");
