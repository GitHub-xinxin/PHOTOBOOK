...
//trimarray:修剪信息；$data：模板的框图信息；$T_photo:模板图 $img:合成图存放位置 $id:订单页ID，$window_W：手机屏幕宽度
//logid纯粹为了导出各步骤图片，加上订单页order_sub的id号，便于区隔
function createzLeaf($trimarray,$data,$T_photo,$img,$id='',$window_W='',$logid=''){
	load()->func('logging');
	logging_run('开始调用createzLeaf','info','photobook');
	logging_run($trimarray,'trimarray数组','photobook');
	logging_run($data,'data数组','photobook');
	logging_run($id,'订单页id','photobook');
	logging_run($window_W,'window_W值','photobook');
	logging_run('合成图的位置：'.$img,'info','photobook');
	$T_size = getimagesize(tomedia($T_photo));
	$T_size318_height=318/$T_size[0]*$T_size[1];
	logging_run($T_size318_height,'模板背景图处理后的高度','photobook');
	logging_run($T_size,'模板图尺寸-T_size','photobook');
	// 创建一个和模板图宽高一样的画布
	// writelog('进入函数'." empty share");
	// writelog('初始值$trimarray'.$trimarray);
	$target = imagecreatetruecolor(318, $T_size318_height);
	

	
	$testUrl_ti='/www/web/photobook/public_html/attachment/BOOKS/test/temp_book'.$logid.'_ti.png';
	...
	foreach ($data as $key => $invalue) {
		...
		logging_run('进入createzLeaf循环','info','photobook');
		
		if($invalue['type']=='img'){
			logging_run('进入createzLeaf循环的img类型','info','photobook');
			// 用户图
			$U_photo=tomedia($trimarray[$key]['imgurl']);
			if(empty($U_photo)){
				logging_run('订单页id：'.$id.'  key为：'.$key.' invalue'.json_encode($invalue),'error','photobook');
			}
			$U_size = getimagesize($U_photo);
			...
			if(!$id){
				logging_run('id为空','info','photobook');
				//修正trim的值，将来要返回去并更新到ordersub的trim里。这里修正就是改变用户图的高度和宽度，这个宽高度是缩放后的，不是原始的。然后宽高交换-sun
				if($trimarray[$key]['roate']){
					if(abs($trimarray[$key]['roate'])%90==0){
				...
				// 定义选框的宽高
				logging_run('开始步骤2','info','photobook');
				/* $inxy=array(//模板框处理后的尺寸（invalue是框原尺寸，inxy是处理后的尺寸）
					318/$T_size[0]*(float)$invalue['width'],//把模板图缩放到318像素宽的情况下，模板上框的宽度
					318/$T_size[0]*(float)$invalue['height'],//高度同上处理
				); */
				
				//之所以没有缩放，是因为发现在web端编辑时，模板页所在div宽度就是318，不知为何。注意框的宽度和模板背景实际大小无关，而是显示大小有关
				$inxy=array((float)$invalue['width'],(float)$invalue['height']);
				logging_run((float)$invalue['width'],'测试转换'.$key.':','photobook');
				logging_run($invalue,'缩放前，框图尺寸-invalue'.$key.':','photobook');
				logging_run($inxy,'缩放后，框图尺寸-inxy'.$key.':','photobook');
				
				//遍历步骤3：根据当前框的尺寸，按照用户图比小边填满框的原则，缩放用户图的尺寸，这个和318无直接关系，主要原则是用户图弱边填满框
				if($inxy[0]/$inxy[1]>$U_size[0]/$U_size[1]){
					$w=$inxy[0]; 
					$h=$U_size[1]*($inxy[0]/$U_size[0]); 
				}else{
				...
				logging_run($klt,'缩放后图片相对于框的位置XY-klt:',"photobook");
		...
		/* 		$xy=array(
				...
				$testUrl='/www/web/photobook/public_html/attachment/BOOKS/test/temp_book'.$logid.'_k'.$key.'.png';
				imagepng($kuang,$testUrl);
				$testUrl_t1='/www/web/photobook/public_html/attachment/BOOKS/test/temp_book'.$logid.'_tb'.$key.'.png';
				imagepng($target,$testUrl_t1);
				imagecopy($target, $kuang,$xy['x'],$xy['y'], 0, 0,$inxy[0], $inxy[1]);
				$testUrl_t2='/www/web/photobook/public_html/attachment/BOOKS/test/temp_book'.$logid.'_ta'.$key.'.png';
				imagepng($target,$testUrl_t2);
				imagedestroy($kuang);
				logging_run('位置调制的图片，在此整合完毕','info',"photobook");
			}else{// 程序图片自适应
...
/* 				$xy=array(
				...
				logging_run($klt,'缩放后图片相对于框的位置XY-klt-自动计算:',"photobook");
				...
				//声明一个和框一样的画布，用来保存缩放用户图的。
				$kuang=imagecreatetruecolor($inxy[0], $inxy[1]); 
				// 关键函数，参数（目标资源，源，目标资源的开始坐标x,y, 源资源的开始坐标x,y,目标资源的宽高w,h,源资源的宽高w,h） 
				// imagecopyresampled($kuang, $resize_user_img, 0, 0, ($w-$inxy[0])/2, ($h-$inxy[1])/2, $w, $h, $U_size['0']-($w-$inxy[0])/2, $U_size['1']-($h-$inxy[1])/2); 
				imagecopyresampled($kuang, $resize_user_img, 0, 0, $klt['x'], $klt['y'], $w, $h, $w,$h); 
				imagedestroy($resize_user_img);
				// 导出放大缩小后的用户图
				// imagepng($kuang, ATTACHMENT_ROOT."BOOKS/user".uniqid().".png");
				// 把缩放后的用户图放到画布上，选框位置上
				imagecopy($target, $kuang,$xy['x'],$xy['y'], 0, 0,$inxy[0], $inxy[1]);
				imagedestroy($kuang);
				// echo '内层循环完成';
	...
	mergeImage_thumb($target,tomedia($T_photo),array('left'=>0,'top'=>0,'width'=>318,'height'=>$T_size318_height),$logid);
/* 	$img_template = imagecreates($T_photo);
	$temp_width = imagesx($img_template);
	$temp_height = imagesy($img_template);
	logging_run('开始缩小模板页背景图','info','photobook');
	$img_template_scaled=imagecreate(318,$T_size318_height);
	$color=imagecolorallocate($img_template_scaled,255,255,255); 
	imagecolortransparent($img_template_scaled,$color); 
	imagefill($img_template_scaled,0,0,$color); 
	logging_run('颜色透明添加完成','info','photobook');
	imagecopyresized($img_template_scaled,$img_template,0,0,0,0,318,$T_size318_height,$temp_width,$temp_height);
	imagecopyresized($target, $img_template_scaled, 0,0, 0, 0,318,$T_size318_height,318,$T_size318_height);
	logging_run('模板页和框图页整合完毕','info','photobook'); */
	...
	if(!$id){
		// 人为调整后的生成
		// exit;
		// writelog('返回值$trimarray'.$trimarray);
		return $trimarray;//返回处理后的trimarray，该值可能根据318的默认手机屏幕宽处理过
	}else{
		// 默认方式生成
		// 更新数据库
...
//此函数经过改造，先让imgurl图片缩放的和target一样大
 function mergeImage_thumb($target, $imgurl , $data,$id) {
	...
	$w = imagesx($img);
	$h = imagesy($img);
	$img_src_resized=imagecreatetruecolor($data['width'],$data['height']);
