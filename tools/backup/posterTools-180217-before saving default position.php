...
function createzLeaf($trimarray,$data,$T_photo,$img,$id='',$window_W='',$logid=''){
	...
	foreach ($data as $key => $invalue) {
		...
		if($invalue['type']=='img'){
			...
			if(!$id){//id为空就取值，不为空就默认计算
				logging_run($id,'此时模板页id值为空，从数据库取Xleft和XTop值:',"photobook");
				$klt=array(
				'x'=>abs(318/$window_W*trim($trimarray[$key]['Xleft'],'px')),
				'y'=>abs(318/$window_W*trim($trimarray[$key]['Xtop'],'px')),
				);
			}else{
				logging_run($id,'此时模板页id值不为空，系统按默认值计算位置，id为:',"photobook");
				$klt=array(
				'x'=>-abs($inxy[0]-$w)/2,
				'y'=>-abs($inxy[1]-$h)/2,
				);
	...
	if(!$id){
		...
		logging_run($xy,'准备更新照片页的Trim值:',"photobook");
