...
/*	public function doWebPost_ser_msg(){
	...
	public function checkLimits($type,$list){
		...
		logging_run('在checklimits中，type和framecount分比为：'.$type.', frame1:'.$frameCount1.$frameCount2.$frameCount3.$frameCount4.$frameCount5.$frameCount6,'info','photobook');
	...
	//填充封面封底
	public function fillCovers($userPics,$tid){
		global $_W,$_GPC;
	...
	public function  doMobileAPI_SaveBook(){
		...
		$modeResult=pdo_fetch('SELECT * FROM ims_ly_photobook_modes WHERE userpics=:userpics',array(':userpics'=>$uimgcount-2));//封面封底用了两张
		if(empty($modeResult)){
		...
		while($imgIndex<$uimgcount && $safeValve<21 && $imgIndex<$userPicMax){//只要客户照片没用完，并且未达到安全阀值，就继续循环，直到把用户照片用完	
			$safeValve=$safeValve+1;
			foreach ($temps as $key => $tep) {
				$this_uimgcount=$tep['imagecount'];
				if($modeResult['frame'.$this_uimgcount]>0){
					continue;
				}else{
					$modeResult['frame'.$this_uimgcount]=$modeResult['frame'.$this_uimgcount]-1;//预设排版样式中，该种框图消耗了一个
				}
				// 组织数据结构
				if($uimgcount-$imgIndex<$this_uimgcount){//说明客户的图片不够用了//此问题不应该出现，出现则提示并停止.注意照片从0开始，所以要加1，由于本页可用，所以又要减1
				...
				for($i=0;$i<$this_uimgcount;$i++){//this_uimgcount模板上框的数量
					$img=$arrs[$imgIndex];
					$imgIndex+=1;
					if($imgIndex>=$uimgcount){//由于是两层循环，所以内部循环也得加
