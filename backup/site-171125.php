	...
	public function doWebCreateBook(){
		...
		if(checksubmit()){
			...
			if(pdo_insert('ly_photobook_template_main',$data)){
				message('添加成功，去添加模板图',$this->createWebUrl('uploadtemplate',array('template_id'=>pdo_insertid())),'success');
			}else{
				message('添加失败','','error');
			}
			exit;
 ...
 */
	...
	public function doWebEdittemplateimage(){
		...
		$sql='select thumb,id from '.tablename('ly_photobook_template_sub').' where template_id=:template_id AND uniacid=:uniacid';
		$images=pdo_fetchall($sql,array('template_id'=>$_GPC['t_id'],'uniacid'=>$_W['uniacid']));
	/*	$originals=array();
	...
	public function doWebEditimage(){
		...
		if(checksubmit()){
			...
			if(empty($_GPC['template_id'])&&!empty($_GPC['type'])&&!empty($_GPC['t_id'])){
				...
				message('操作成功',$this->createWebUrl('photobook'),'success');
			}
			$data=array(
					'original'=>$_GPC['bg'],
					'imagecount'=>$_GPC['imagecount'],
...
/*	public function doWebPost_ser_msg(){
	...
	public function doMobileNewimgtouchit(){
		...
		$Tlist=pdo_getall("ly_photobook_template_sub",array('template_id'=>$template_id),array('id','thumb'));
