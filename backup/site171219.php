...
/*	public function doWebPost_ser_msg(){
	...
	public function  doMobileAPI_SaveBook(){
		...
		$tid=$_GPC['tid'];
		//取出模板的类型
		$templateMain=pdo_get('ly_photobook_order_main',array('id'=>$tid),array('shell'));
		if(empty($templateMain)){
			message('参数错误或模板有误，请联系管理员','','error');
			exit;//一个防火墙而已！
		}
		...
		//此处先判断模板中，每种框数的子模板必须大于1（对于软壳和硬壳，方款，要求不一样）		
		...
		$imgIndex=0;
		$usedup=0;
		$safeValve =0; //安全阀值，当前最多为20，避免模板设置失误或其它因素，造成大量循环
		
		//根据客户照片数取出对应的预设排版模式
		$modeResult=pdo_fetch('SELECT * FROM ims_ly_photobook_modes WHERE userpics=:userpics',array(':userpics'=>$uimgcount));
		if(empty($modeResult)){
			message('找不到这种照片书对应的排版组合，请联系管理员','','warnning');
			exit;
		}
		...
		foreach ($teps as $key => $tep) {
			...
			for($i=0;$i<$this_uimgcount;$i++){//this_uimgcount模板上框的数量
				if($uimgcount-$imgIndex<$this_uimgcount){//说明客户的图片不够用了
					$usedup=1;
					break;
				}	
				$img=$arrs[$imgIndex];
			...
			if($usedup==1)//客户的图片不够用了，紧接着退出，不要插入order_sub记录了
				break;
