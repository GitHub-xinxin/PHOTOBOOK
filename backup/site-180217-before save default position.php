...
/*	public function doWebPost_ser_msg(){
	...
	public function doMobileAPI_Texttext(){
		...
		load()->func('logging');
		logging_run('前端参数：'.$window_W,'info',photo);
		...
		$trimarray=createzLeaf($trimarray,$data,$template['thumb'],$img,'',318,$order_id);//原来是$window_W，该数据为360，不如统一定为318
		if($_GPC['NewTid']!=-1){
			pdo_update('ly_photobook_order_sub',array('trim'=>json_encode($trimarray),'template_id'=>$_GPC['NewTid']),array('id'=>$order_id));
		}else{
			$res=pdo_get('ly_photobook_order_sub',array('id'=>$order_id))['trim'];
			$this->writelog('初始trim值'.$res);
			$ress=pdo_update('ly_photobook_order_sub',array('trim'=>json_encode($trimarray)),array('id'=>$order_id));	
			$re =pdo_get('ly_photobook_order_sub',array('id'=>$order_id))['trim'];
			$this->writelog('更新后trim值'.$re);
		}
